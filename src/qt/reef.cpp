...
{
    return QCoreApplication::translate("reef-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in reef.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in reef.qrc)
...
/** Class encapsulating Proton Core startup and shutdown.
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(reef);
    Q_INIT_RESOURCE(reef_locale);
    ...
    /// 6. Determine availability of data directory and parse reef.conf
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("Proton Core"), QObject::tr("Error: %1").arg(e.what()));
        return EXIT_FAILURE;
    }
#ifdef ENABLE_WALLET
    ...
    // reef: links repeatedly have their payment requests routed to this process:
