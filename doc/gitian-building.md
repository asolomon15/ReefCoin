...
*Setup instructions for a Gitian build of Proton Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Proton
...
- [Building Proton Core](#building-reef-core)
...
Clone the git repositories for Proton Core and Gitian.
...
in the Proton Core repository under 'Fetch and build inputs' to install sources which require
...
To build Proton Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Proton Core repository.
